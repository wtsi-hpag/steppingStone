...
{
    FILE *namef,*namef2;
    int i=0,j=0,k,args=0,num_break2,nSeq,nRead,num_stones;
    int edge1,edge2;
    int *stn_chrs1,*stn_chrs2,*stn_reads,*stn_read2,*stn_offset4,*stn_index;
...
/*  read the alignment files         */
    i=0;
    num_stones = 0;
    while(fscanf(namef,"%d %d %d %d %d %d %d %s %s %d %d %d %d",&stn_index[i],&stn_chrs1[i],&stn_loci1[i],&stn_chrs2[i],&stn_loci2[i],&stn_reads[i],&stn_read2[i],tmpc,rdname[i],&stn_offset1[i],&stn_offset2[i],&stn_offset3[i],&stn_offset4[i])!=EOF)
    {
      i++;
    }
    fclose(namef);
...
//         printf("www: %d %d %d %d\n",i,stn_loci2[i],stn_loci2[j],BAR);
	 ...
	 edge2 = 0;
	   ...
	   edge2 = 2;
           ...
           edge2 = 1;
         fprintf(namef,"%d %d %d %d %d %d || %d %d %s\n",num_stones,stn_chrs1[i],stn_loci1[i],stn_chrs2[i],stn_loci2[i],j-i,edge1,edge2,rdname[i]);
         num_stones++;
       }
       i=j-1;
    }
