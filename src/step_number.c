...
int main(int argc, char **argv)
{
    ...
    char *ptr,**ctgname,cigarline[500000],*seqline,*read_base,*refe_base;
...
//        printf("System command error:\n);
    ...
    if((line= (char *)calloc(2000000,sizeof(char))) == NULL)
    {
      printf("ERROR Memory_Allocate: calloc - line\n");
      exit(1);
    }
    if((line2= (char *)calloc(2000000,sizeof(char))) == NULL)
    {
      printf("ERROR Memory_Allocate: calloc - line\n");
      exit(1);
    }
    if((line3= (char *)calloc(2000000,sizeof(char))) == NULL)
    {
      printf("ERROR Memory_Allocate: calloc - line\n");
      exit(1);
    }
/*  input read alignment info line   */
    ...
    while(!feof(namef))
    {
      if(fgets(line,2000000,namef) == NULL)
...
//           printf("%s %s %d\n",ctgname[num_align],refname,num_index);
             fprintf(namef2,"%s %d %s %d %d %s %s\n",ctgname[num_align],num_index,refname,hitst,60,cigarline,seqline);
           }
        }
      }
