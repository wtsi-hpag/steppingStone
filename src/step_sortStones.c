...
/*  read the alignment files         */
    i=0;
    num_samples = 0;
    while(fscanf(namef,"%s %d %s %s %d %d %d",tmpc,&seq_index[i],tmpc,rdname[i],&ref_offset[i],&seq_offset[i],&seq_len[i])!=EOF)
    {
...
//         printf("www: %d %d %d %d\n",i,ref_offset[i],ref_offset[j],BAR);
	 ...
	 for(k=i;k<j;k++)
            fprintf(namef,"%d %d %s %d %d %d\n",num_steps,k,rdname[k],ref_offset[k],j-i,seq_index[k]);
         num_steps++;
       }
       i=j-1;
    }
