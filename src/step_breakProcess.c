...
int main(int argc, char **argv)
{
    ...
    int *ref_offset,*seq_offset,*seq_len,*seq_index,*seq_rcdex,nstep = 0,stopflag;
    ...
    if((seq_len= (int *)calloc(nRead,sizeof(int))) == NULL)
    {
      printf("ERROR Memory_Allocate: Align_Process - hit_rcdex\n");
      exit(1);
    }
    ...
    if((seq_rcdex= (int *)calloc(nRead,sizeof(int))) == NULL)
    {
      printf("ERROR Memory_Allocate: Align_Process - hit_rcdex\n");
      exit(1);
    }
...
/*  read the alignment files         */
    i=0;
    num_samples = 0;
    while(fscanf(namef,"%s %s %s %s %d %d %d",tmpc,tmpc,tmpc,rdname[i],&ref_offset[i],&seq_offset[i],&seq_len[i])!=EOF)
    {
...
/*  read the alignment files         */
    ...
    {
      fprintf(namef2,"%s %d %s %s %d %d %d\n",tmpc1,seq_index[i],tmpc2,tmpc3,offset1,offset2,offset3);
      i++;
    }
    fclose(namef);
    fclose(namef2);
