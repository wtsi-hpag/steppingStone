...
int main(int argc, char **argv)
{
    ...
    int *ref_offset,*seq_offset,*seq_len,*seq_index,*chr_index,nstep = 0,stopflag;
...
/*  read the alignment files         */
    i=0;
    num_samples = 0;
    while(fscanf(namef,"%s %d %d %s %d %d %d",tmpc,&seq_index[i],&chr_index[i],rdname[i],&ref_offset[i],&seq_offset[i],&seq_len[i])!=EOF)
    {
...
//         printf("www: %d %d %d %d\n",i,ref_offset[i],ref_offset[j],BAR);
	 ...
	 for(k=i;k<j;k++)
            fprintf(namef,"%d %d %s %d %d %d %d %d %d\n",num_steps,k,rdname[k],chr_index[k],ref_offset[k],j-i,seq_index[k],seq_offset[k],seq_len[k]);
         num_steps++;
       }
       i=j-1;
    }
